...
//int main()
{
...
#ifdef __cplusplus
  std::vector<std::string> files(1, "rhf.fcidump");
  files.emplace_back("uhf.fcidump");
#else
  ...
  for (ifile = 0; ifile < 2; ifile++) {
    if (parallel_rank == 0) {
      ...
      std::cout << "ORBSYM=";
      for (std::vector<int>::const_iterator s = ORBSYM.begin(); s < ORBSYM.end(); s++) std::cout << *s << ",";
      std::cout << std::endl;
      int i, j, k, l;
      double value;
...
//        std::cout << "type "<<type<<FCIdump::I1a<<FCIdump::I1b<<std::endl;
      ...
      std::vector<std::string> testc(1, "99");
      testc.emplace_back("98");
